from django.urls import path
from main.views import show_main, show_xml, show_json, show_xml_by_id, show_json_by_id, add_product, show_product 
from main.views import register, login_user, logout_user, edit_product, delete_product
from main.views import login_ajax,logout_ajax,register_ajax, add_product_entry_ajax, edit_product_ajax, delete_product_ajax
from main.views import proxy_image, add_product_flutter, get_user_products_json

app_name = 'main'

urlpatterns = [
    path('', show_main, name='show_main'),
    path('xml/', show_xml, name='show_xml'),
    path('json/', show_json, name='show_json'),
    path('xml/<uuid:product_pk>/', show_xml_by_id, name='show_xml_by_id'),
    path('json/<uuid:product_pk>/', show_json_by_id, name='show_json_by_id'),
    path('add-product/', add_product, name='add_product'),
    path('product/<uuid:pk>/', show_product, name='show_product'),
    path('register/', register, name='register'),
    path('login/', login_user, name='login'),
    path('logout/', logout_user, name='logout'),
    path('product/<uuid:id>/edit', edit_product, name='edit_product'),
    path('product/<uuid:id>/delete', delete_product, name='delete_product'),
    path('create-product-ajax', add_product_entry_ajax, name='add_product_entry_ajax'),
    path('login-ajax/', login_ajax, name='login_ajax'),
    path('register-ajax/', register_ajax, name='register_ajax'),
    path('logout-ajax/', logout_ajax, name="logout_ajax"),
    path('product/<uuid:id>/edit_ajax', edit_product_ajax, name='edit_product_ajax'),
    path('product/<uuid:id>/delete_ajax', delete_product_ajax, name='delete_product_ajax'),
    path('proxy-image/', proxy_image, name='proxy_image'),
    path('add-flutter/', add_product_flutter, name='add_product_flutter'),
    path('json-user/',get_user_products_json,name='get_user_products_json'),
]